...
class category extends Component {
    ...
    _addCategory = async function() {
        ...
        if(category_name !== '' && category_name.length > 0) {
            ...
            add.data.suc ? console.log('hi') : alert(add.data.msg);
            this._getCategoryData();
    ...
    render() {
        ...
        return (
            <div className='Category'>
                <ul>
                    <li><Link className={pre_cat === '' ? "pre_cat" : null} to='/' onClick={() => _changeCategory('')}> 전체 보기 </Link> 
                    {login && admin ==='Y' && user_ip === "192.168.43.69" ? !edit ? <input type='button' value='Edit' className='Edit' onClick={() => this.setState({ edit : !edit })} /> 
